    ...
    class Program
    {
        ...
        protected static bool CheckCapitalizationModifiers(string story, out string invalidEntry)
        {
            ...
            foreach(Match match in matches)
            {
                ...
                Regex captRegex = new Regex(@"^(?:city|town|country|first name|last name|celebrity|female celebrity|male celebrity|name of a person in the room|name of a girl in the room|name of a boy in the room|famous person|actor)");
                if (captRegex.IsMatch(cleanedWord))
                {
                    //isvalid
                    string modifier = match.Groups["Modifier"].Value;
                    if(!Regex.IsMatch(modifier, @"^(?:C|AC|SW\d+\|C|SW\d+\|AC)$")){
                        isValid = false;
                        invalidEntry = match.Groups["Entry"].Value;
                    }
                }
